...
var newResJump=0; // 新着レスジャンプ（0:ブラウザ任せ、1:読了時のみ、2:常時）＊かちゅ、OpenJane、twintailのみの設定
//==========以下はスクリプト本文ですよ。
//イベントハンドラ定義
//外部関数：tohan
...
function mouseMoveEvent() {
 if(!cp){
  var nHTML ='<div id="controlPanel">'
            + '<input type="button" value="IMG" onmouseup="imgCommand();blur()">'
            + '<input type="button" value="FND" onclick="fndCommand();blur();">'
            +'</div>'
            +'<div id="foundPanel" onmouseout="searched=false"></div>';
  if(skinName.match(/30-2/)){
    nHTML+='<div id="thumbPanel" onmouseout="viewed=false"></div>'
...
function onscrollEvent(){mouseMoveEvent();searchColoring();}
if(document.getElementById("dl")){
 document.getElementById("dl").onscroll=function(){onscrollEvent()}
}
//=========onLoad処理
window.onload=onLoadEvent;
function onLoadEvent(){loadtime();mouseMoveEvent();setHash();searchColoring();}
