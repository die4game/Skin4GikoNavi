...
function changeMode(){
 ...
 if(lightmode){//normalへ
  ...
  /* panel=on   */	panel=true;
  // thumbPanelのスタイル変更
  if(tp){tp.style.height=document.body.clientHeight-25}
 }else{ //lightへ
 ...
 if(a.className!='replaced' && a.parentElement.tagName!='DIV'){
  if(imageExt2(a.href)){
   ...
   var nHTML='<input type="button" value="VIEW" onClick=\'changeView("html","'+ahref+'");blur()\'>'
            +'<input type="button" value="CHK" onClick=\'changeView("html","http://www.aguse.jp/?url='+ahref+'");blur()\'>';
  }
  a.insertAdjacentHTML('AfterEnd',nHTML);
  a.className = 'replaced';
  if(imageExt2(a.href)){
   if(!event||event.type=="mousemove"||event.type=="scroll"){ // autoImageLoadでの読込
...
function changeView(mode,href){
 ...
 if(!vp.hasChildNodes() || vp.firstChild.src!=href){
  ...
  if (mode=="img") {
    nHTML='<img src="'+href+'" ondragstart="imageMoveStart(this)" ondrag="imageMove(this)" onclick="clearView()">';
