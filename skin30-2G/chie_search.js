...
function setHash(){
 ...
 if(isNaN(lastNum)&&l>1){lastNum=parseInt(dts[l-2].firstChild.firstChild.innerText)} //Footerありなら一つ前 lastNumに最終レス 番号を取得
 ...
 for(var i=setEndIdx;i<l;i++){
  ...
  if(!dts[i].firstChild.firstChild){continue} // レス番が取得できなければ回避
...
function setSearchColor(dt){
 if(!dt.innerText||!dt.firstChild.firstChild){return}// dtが空またはdtがレスヘッダではないときはリターン
 // ID
 ...
 // ref
 var numA=dt.firstChild.firstChild; var num=parseInt(numA.innerText); 
 if(refHash[num]){
  var refResult=refHash[num].split(" ");
  if     (refResult.length>=threshold6[0]){numA.style.color=threshold6[1];dt.firstChild.title="これへのレス ("+refResult.length+"個)\n"+refHash[num];}
  else if(refResult.length>=threshold5[0]){numA.style.color=threshold5[1];dt.firstChild.title="これへのレス ("+refResult.length+"個)\n"+refHash[num];}
  else if(refResult.length>=threshold4[0]){numA.style.color=threshold4[1];dt.firstChild.title="これへのレス ("+refResult.length+"個)\n"+refHash[num];}
  else{dt.firstChild.title="これへのレスはない模様"}
 }else{dt.firstChild.title="これへのレスはない模様"}
}
//==========名前・ID検索等
...
function createSearchPanel(arg){ // 専用ブラウザでのmodelessDialogへの外部読み込みでは、dialogArgumentsが送れないので直接書き出す
 ...
 var nWin='<html><head><title>単語の検索<\/title>'
         ...
         +'<label for="word">検索文字列：</label><input type="text" name="word" id="word" size="30" value="'+word+'" tabindex="0"><br>'
...
function setHighlight(reg){
 ...
 while(range.findText(reg)){
  ...
  else if(tag=="SPAN"&& dt.className=="r")  {thisRange="id"}
...
function copyText(target){
 ...
 else if(target=="name"){textarea.value=obj.childNodes[0].childNodes[1].innerText+"\n";}
