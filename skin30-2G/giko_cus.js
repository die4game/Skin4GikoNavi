...
function checkAnchor(href){if(!href){return(0)}
 if     (href.match(/decoy:|about:/)){return(1)}
...
function hiddenRes(e){ 
 ...
 var i=0;while(document.anchors[i].name.match(/\D/)){i++}
 var startRes=parseInt(document.anchors[i+1].name.replace(/\D/g,""));
 if(number.match(/(\d*)\D+(\d*)/)){var start=parseInt(RegExp.$1);}
 else {var start=parseInt(number);}
 //alert(startRes+">"+start+" && "+start+"!="+1);
 if(startRes>start && start!=1){return true}else{return false}
}
//==========スレのロードにかかった時間の表示
function loadtime() {
 var end_time = new Date();
 var start_num=start_time.getMilliseconds()+1000*(start_time.getSeconds()+60*start_time.getMinutes()+3600*start_time.getHours());
 var end_num  =end_time.getMilliseconds()  +1000*(end_time.getSeconds()  +60*end_time.getMinutes()  +3600*end_time.getHours());
 var d = document.getElementById("sizeinfo");
 var s = d.getElementsByTagName("span");
 s[0].innerHTML += (end_num-start_num)/1000+'秒';
}
