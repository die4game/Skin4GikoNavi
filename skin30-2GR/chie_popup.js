...
function namePopup(e, before, num, after) {
  var hnum = tohan(num);
  if (hnum == 774 || hnum == 21 || hnum == 1 || hnum == 30) {
    return
  } // Socket774、774KB、21禁、[1-30]、などは無視
  if (before && before.match(/名無/)) {
    return
  } //名無しっぽいのは無視
  if (after) {
    if (after == "周年") {
  ...
  var nB = before ? "<b>" + before + "</b><b>" : "<b>";
  nB += addAnchor(hnum, num);
  nB += after ? "</b><b>" + after + "</b>" : "</b>";
  e.outerHTML = nB;
}
...
function abonePopup(e) {
  showModelessDialog(document.getElementsByName("ThreadURL")[0].content.replace(/\/l50$/, "/") + e.parentElement.previousSibling
    .innerText, window, "dialogWidth:" + document.body.clientWidth +
...
function searchPopup(obj) {
  ...
  /*ref Popup*/
  if (tag == "A") {
    if (refHash[obj.innerText]) {
      cArray = refHash[obj.innerText].split(" ")
    } else {
      return
    }
  }
  ...
  /*検索Popup*/
    ...
    var cHash = (obj.parentElement.rel == "res") ? foundRes : highlight;
    cArray = cHash[obj.parentElement.firstChild.style.backgroundColor];
    if (!cArray) {
      return
    }
  }
  ...
  for (var i = 0; i < cArray.length; i++) {
    targetString += returnString(cArray[i]);
  } //中身の切り出し
  if (targetString) {
    popup(targetString);
  } //対象が存在したらポップアップ
...
function makePopContent(obj) {
  ...
  for (var i = 0; i <= end - start; i++) {
    targetString += returnString(start + i)
  }
...
function returnString(num) {
  ...
  if (dd.hasChildNodes()) {
    while (dd.lastChild.name) {
      dd.lastChild.removeNode(true)
    } // LABELNUMBER非対応のものに係るリンクアンカーの除去
    if (dd.lastChild.tagName == "DL") {
      dd.lastChild.removeNode(true)
    } // 逆参照の除去
  }
...
function popup(inner) {
  ...
  if (pos == event.y) {
    ...
    var y = scTop - p.clientHeight;
  } // カーソルの上へ表示
  if (y < 0) {
    y = 0
  }
  if (event.srcElement.parentElement.tagName == "TT") {
    y += 35;
  }
  p.style.pixelTop = y;
  // x軸調整
  ...
  if (p.clientHeight > Math.max(event.y, document.body.clientHeight - event.y)) {
    if (p.clientHeight > document.body.clientHeight) {
      p.style.pixelHeight = document.body.clientHeight - 2;
    }
